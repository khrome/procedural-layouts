...
class Adventure {
  ...
  generate() {
    ...
    for (let i = 0; i < this.maxRooms - 1; i++) {
      let dir = DIR[Math.floor((0, _random.random)() * DIR.length)]; // Which direction are we adding a room from

      let x, y, parentRoom;
      if (dir === 'n') {
        // Sliding southward from north
        x = Math.floor((0, _random.random)() * (this.bounds.e - this.bounds.w)) + this.bounds.w;
        const parentRoomId = this.furthest.n.get(x);
        parentRoom = this.rooms[parentRoomId];
        y = parentRoom.y - 1;
      } else if (dir === 'e') {
        // Sliding westward from east
        y = Math.floor((0, _random.random)() * (this.bounds.s - this.bounds.n)) + this.bounds.n;
