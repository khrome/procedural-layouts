...
class Roguelike {
   ...
   */
  generateRoom() {
    var slide = randomFn(0, 3); // North, East, South, West
  ...
  addDoorBetweenRooms(x_dir, y_dir, existing_room_id, new_room_id) {
    ...
    if (x_dir === 1) {
      ...
      y = randomFn(Math.max(existing_room.top, new_room.top) + 1, Math.min(existing_room.top + existing_room.height, new_room.top + new_room.height) - 2);
      orientation = 'h'; // horizontal
    } else if (x_dir === -1) {
      // stabbing westward
      x = new_room.left - 1;
      y = randomFn(Math.max(new_room.top, existing_room.top) + 1, Math.min(new_room.top + new_room.height, existing_room.top + existing_room.height) - 2);
      orientation = 'h'; // horizontal
    } else if (y_dir === -1) {
      // northward
      x = randomFn(Math.max(existing_room.left, new_room.left) + 1, Math.min(existing_room.left + existing_room.width, new_room.left + new_room.width) - 2);
      y = new_room.top - 1;
      orientation = 'v'; // vertical
    } else if (y_dir === 1) {
      // southward
      x = randomFn(Math.max(new_room.left, existing_room.left) + 1, Math.min(new_room.left + new_room.width, existing_room.left + existing_room.width) - 2);
  ...
  randomNonEdgeInRoom(room_id) {
    ...
    return {
      x: randomFn(room.left + 1, room.left + room.width - 2),
      y: randomFn(room.top + 1, room.top + room.height - 2)
    };
  }
...
const roguelike = function (config) {
  var level = new Roguelike(config);
  return level.build();
};
exports.roguelike = roguelike;
function shuffle(o) {
  for (var j, x, i = o.length; i; j = Math.floor((0, _random.random)() * i), x = o[--i], o[i] = o[j], o[j] = x);
}
 ...
 */
function randomFn(min_raw, max_raw) {
  var min = Math.floor(min_raw);
  var max = Math.floor(max_raw);
  return Math.floor((0, _random.random)() * (max + 1 - min) + min);
}
